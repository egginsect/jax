...
# The papply implementation has two parts:
#   - parallel_xla_call, which is analogous to xla.xla_call, and
#   - papply_transform, which is used underneath the parallel_xla_call impl.

# First, papply_transform is a final interpreter transformation that propagates
# the sharded axis and decides, with a greedy forward-pass heuristic, how to
# shard intermediate values given axes along which the inputs are to be sharded.

@lu.transformation_with_aux
def papply_transform(axes, *args):
  with new_master(PapplyTrace) as master:
    trace = PapplyTrace(master, core.cur_sublevel())
    in_tracers = map(partial(PapplyTracer, trace), args, axes)
...
class PapplyTracer(Tracer):
  def __init__(self, trace, val, axis):
    self.trace = trace
    self.val = val
    self.axis = axis
...
class PapplyTrace(Trace):
  def pure(self, val):
    return PapplyTracer(self, val, None)

  def lift(self, val):
    return PapplyTracer(self, val, None)

  def sublift(self, val):
    return PapplyTracer(self, val.val, val.axis)

  def process_primitive(self, primitive, tracers, params):
    vals, axes = unzip2((t.val, t.axis) for t in tracers)
    if all(axis is None for axis in axes):
      return primitive.bind(*vals, **params)
    else:
      rule = get_papply_rule(primitive)
      val_out, axis_out = rule(vals, axes, **params)
      return PapplyTracer(self, val_out, axis_out)

  def process_call(self, call_primitive, f, tracers, params):
    raise NotImplementedError  # TODO(mattjj): inside impl!
  ...
  def pack(self, tracers):
    ...
    axis = tuple(t.axis for t in tracers)
    return PapplyTracer(self, vals, axis)


papply_rules = {}

def get_papply_rule(primitive):
  if primitive in papply_rules:
    return papply_rules[primitive]
  else:
    raise NotImplementedError("no papply rule for {}".format(primitive))

def defvectorized(prim):
  papply_rules[prim] = partial(vectorized_papply, prim)

def vectorized_papply(prim, vals, dims, **params):
  assert all(dims[0] == d for d in dims[1:])
  return prim.bind(*vals, **params), dims[0], None


# Second, we implement parallel_xla_call analogous to xla.xla_call.
